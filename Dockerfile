# syntax=docker/dockerfile:experimental

FROM ubuntu:plucky AS builder
LABEL description="Development build environment"

# Update the distro and install our tools
RUN --mount=type=cache,target=/var/lib/apt/lists \
    --mount=type=cache,target=/var/cache/apt/archives \
    apt-get -y update && apt-get -y upgrade && \
    apt-get -y install software-properties-common && \
    apt-get -y install wget && \
    # GCC stuff
    apt-get -y install build-essential gcc-14 g++-14 && \
    # Clang stuff (if needed)
    # wget -qO- https://apt.llvm.org/llvm-snapshot.gpg.key | tee /etc/apt/trusted.gpg.d/apt.llvm.org.asc >/dev/null && \
    # add-apt-repository -y 'deb http://apt.llvm.org/noble/ llvm-toolchain-noble-18 main' && \
    # apt-get -y update && \
    # apt-get -y install clang-18 && \
    update-alternatives --install /usr/bin/cc cc /usr/bin/gcc-14 100 && \
    update-alternatives --install /usr/bin/c++ c++ /usr/bin/g++-14 100 && \
    update-alternatives --install /usr/bin/gcc gcc /usr/bin/gcc-14 100 && \
    update-alternatives --install /usr/bin/g++ g++ /usr/bin/g++-14 100 && \
    apt-get -y install libstdc++-14-dev && \
    apt-get -y install cmake && \
    apt-get -y install git && \
    # Install required library packages
    apt-get -y install libbotan-2-dev && \
    apt-get -y install libmysqlcppconn-dev && \
    apt-get -y install zlib1g-dev && \
    apt-get -y install libpcre3-dev && \
    apt-get -y install libflatbuffers-dev

# Download, extract, and build Boost using its own cache, and force a commit of its cache into a layer
RUN --mount=type=cache,sharing=locked,id=boost,target=/tmp/boost-cache \
    mkdir -p /tmp/boost-cache && \
    if [ ! -f /tmp/boost-cache/boost_1_87_0.tar.gz ]; then \
      wget -q -O /tmp/boost-cache/boost_1_87_0.tar.gz https://archives.boost.io/release/1.87.0/source/boost_1_87_0.tar.gz; \
    fi && \
    tar -zxf /tmp/boost-cache/boost_1_87_0.tar.gz -C /tmp/boost-cache && \
    cd /tmp/boost-cache/boost_1_87_0 && \
    ./bootstrap.sh --with-libraries=system,program_options,headers && \
    ./b2 link=static install -d0 -j$(nproc) cxxflags="-std=c++23" && \
    # **Force the Boost cache to be “committed” into a layer**
    cp -R /tmp/boost-cache /boost-cache-retained && \
    ls -la /boost-cache-retained

# Copy source
ARG working_dir=/usr/src/ember
COPY . ${working_dir}
WORKDIR ${working_dir}

# CMake arguments (can be overridden by docker build arguments)
ARG build_optional_tools=1
ARG pcre_static_lib=1
ARG disable_threads=0
ARG build_type=Rel
ARG install_dir=/usr/local/bin

# Generate Makefile & compile with caching enabled for the build directory
RUN --mount=type=cache,target=${working_dir}/build \
    cmake -S . -B build \
      -DCMAKE_BUILD_TYPE=${build_type} \
      -DCMAKE_INSTALL_PREFIX=${install_dir} \
      -DBUILD_OPT_TOOLS=${build_optional_tools} \
      -DPCRE_STATIC_LIB=${pcre_static_lib} \
      -DDISABLE_EMBER_THREADS=${disable_threads} \
      -DBOTAN_ROOT_DIR=/usr/include/botan-2/ \
      -DBOTAN_LIBRARY=/usr/lib/x86_64-linux-gnu/libbotan-2.so && \
    cd build && make -j$(nproc) install && make test

FROM ubuntu:plucky AS run_environment
ARG install_dir=/usr/local/bin
ARG working_dir=/usr/src/ember
WORKDIR ${install_dir}
RUN apt-get -y update && \
    apt-get install -y libbotan-2-19 && \
    apt-get install -y libmysqlcppconn7v5 && \
    apt-get install -y mysql-client
COPY --from=builder ${install_dir} ${install_dir}
RUN cp configs/*.dist .
COPY ./sql ${install_dir}/sql
COPY ./scripts ${install_dir}
COPY ./dbcs ${install_dir}/dbcs