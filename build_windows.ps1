...
#############################################################
# Set the conan cache to the cached dependencies folder
$env:CONAN_USER_HOME = "C:\a\Ember\Ember\dependencies\conan_cache"
...
# Detect and patch the default profile for Debug and C++26
conan profile detect
$profilePath = (& conan profile path default).Trim()
(Get-Content $profilePath) `
    -replace '^(build_type=).*$', 'build_type=Debug' `
    -replace '^(compiler\.cppstd=).*$', 'compiler.cppstd=26' `
    | Set-Content $profilePath -Force
if (-not (Test-Path $buildDir)) {
    New-Item -ItemType Directory -Path $buildDir | Out-Null
}

# Check if Conan dependencies have already been installed by looking for a key output file
if (-not (Test-Path "$buildDir\conan_toolchain.cmake")) {
    ...
    Write-Host "Conan dependencies already installed. Skipping conan install step."
}
